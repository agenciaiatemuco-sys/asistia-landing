...
const ALL_DEMOS = GROUPS.flatMap((g) =>
  g.demos.map((d) => ({ ...d, group: g.group }))
);

const CATEGORIES = ["Todos", ...GROUPS.map((g) => g.group)];
...
export default function Showroom() {
  ...
  const visibleDemos =
    activeCategory === "Todos"
      ? ALL_DEMOS
      : ALL_DEMOS.filter((d) => d.group === activeCategory);
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                activeCategory === cat
                  ? "bg-alkia text-white"
                  : "bg-white border border-gray-200 text-gray-600 hover:border-alkia/40 hover:text-alkia-dark"
              }`}
            >
              {cat}
            </button>
